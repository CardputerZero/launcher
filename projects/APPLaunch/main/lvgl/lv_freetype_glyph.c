...
static bool freetype_glyph_create_cb(lv_freetype_glyph_cache_data_t * data, void * user_data)
{
    ...
    else if(dsc->render_mode == LV_FREETYPE_FONT_RENDER_MODE_BITMAP_MONO) {
        error = FT_Load_Glyph(face, glyph_index, FT_LOAD_COMPUTE_METRICS | FT_LOAD_TARGET_MONO | FT_LOAD_NO_AUTOHINT);
        if(!error) {
    ...
    if(dsc->render_mode == LV_FREETYPE_FONT_RENDER_MODE_OUTLINE) {
        ...
        if(dsc->style & LV_FREETYPE_FONT_STYLE_ITALIC) {
            ...
            dsc->render_mode == LV_FREETYPE_FONT_RENDER_MODE_BITMAP_MONO) {
