...
static bool freetype_image_create_cb(lv_freetype_image_cache_data_t * data, void * user_data)
{
    ...
    if(dsc->render_mode == LV_FREETYPE_FONT_RENDER_MODE_BITMAP_MONO) {
        error = FT_Load_Glyph(face, data->glyph_index, FT_LOAD_TARGET_MONO | FT_LOAD_NO_AUTOHINT);
    }
    else {
    ...
    error = FT_Render_Glyph(face->glyph, dsc->render_mode == LV_FREETYPE_FONT_RENDER_MODE_BITMAP_MONO
                            ? FT_RENDER_MODE_MONO
    ...
    uint32_t pitch_abs = pitch < 0 ? (uint32_t)-pitch : (uint32_t)pitch;
    ...
    for(int y = 0; y < box_h; ++y) {
        const uint8_t * src = pitch < 0
                              ? glyph_bitmap->bitmap.buffer + (box_h - 1 - y) * pitch_abs
                              : glyph_bitmap->bitmap.buffer + y * pitch_abs;
        uint8_t * dst = (uint8_t *)(data->draw_buf->data) + y * stride;

        if(glyph_bitmap->bitmap.pixel_mode == FT_PIXEL_MODE_MONO) {
            for(int x = 0; x < box_w; ++x) {
                dst[x] = (src[x >> 3] & (0x80 >> (x & 0x7))) ? 0xff : 0x00;
            }
        }
        else {
            lv_memcpy(dst, src, LV_MIN(stride, pitch_abs));
        }
    }
