...
#define LV_OBJ_ADD_EVENT_CB(obj, ecode) \
    do{ \
        lv_event_dsc_t *dsc = lv_obj_add_event_cb(obj, LvglCallback, ecode, nullptr); \
        lv_event_desc_set_external_data(dsc, lv_malloc(sizeof(LvglCallback_t)), lv_free); \
        ((LvglCallback_t *)dsc->ext_data.data)->pSelf = this; \
        ((LvglCallback_t *)dsc->ext_data.data)->Code = ecode; \
    }while(0);

struct LvglCallback_t
{
    void *pSelf;
...
public:
    Componens() = default;
    virtual ~Componens() = default;
    virtual void handle_key(lv_event_t *e) = 0;
    virtual void on_clicked(lv_event_t *e) = 0;
    virtual void on_released(lv_event_t *e) = 0;
    virtual void on_value_changed(lv_event_t *e) = 0;
};

extern "C" {
    void LvglCallback(lv_event_t * e)
    {
        LvglCallback_t *callback = (LvglCallback_t *)e->ext_data.data;
        auto *self = static_cast<Componens *>(callback->pSelf);
        switch (callback->Code)
        {
        case LV_EVENT_CLICKED: self->on_clicked(e);break;
        case LV_EVENT_RELEASED: self->on_released(e);break;
        case LV_EVENT_VALUE_CHANGED: self->on_value_changed(e);break;
        default:
            break;
        }
...
class HelloWorldComponens : public Componens
{
public:
    /**
...
class ButtonComponens : public Componens
{
public:
    virtual void on_clicked(lv_event_t *e) override
    {
    ...
    void create_ui(lv_obj_t * parent)
    {
        ...
        LV_OBJ_ADD_EVENT_CB(btn, LV_EVENT_CLICKED);
...
class LvExampleGetStarted4
{
public:
    lv_obj_t * label;

    virtual void on_value_changed(lv_event_t *e) override
    {
    ...
    void create_ui(lv_obj_t * parent)
    {
        ...
        LV_OBJ_ADD_EVENT_CB(slider, LV_EVENT_VALUE_CHANGED);
...
class LvExampleImagebutton1
{
public:
    LvExampleImagebutton1()
    {
        create_ui(lv_screen_active());
    }
    void create_ui(lv_obj_t * parent)
...
public:
    ...
    inline static lv_obj_t * create_slider(lv_obj_t * parent,
                                    const char * icon, const char * txt, int32_t min, int32_t max, int32_t val);
    inline static lv_obj_t * create_switch(lv_obj_t * parent,
