...
| `--service-scope` | `user` | Installs a systemd user service under `/usr/lib/systemd/user`; `system` installs a root service under `/usr/lib/systemd/system` |
| `--service-restart` | `always` | systemd `Restart=` policy for the generated service |
...
With the default `--service-scope user`, the script generates:
...
The default package installs a user service under `/usr/lib/systemd/user`, not a root-owned system service. It is enabled for the UID 1000 user by `postinst`; device permissions for framebuffer, evdev, GPIO, audio, and camera must therefore be provided by the image's user/group rules. When `--service-scope system` is used, the service is installed as `/usr/lib/systemd/system/APPLaunch.service` and uses `WantedBy=multi-user.target`.
