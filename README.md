...
The repository contains three main projects and several example/helper projects:
...
- **ZClaw** - A keyboard-first LVGL chat client for ZeroClaw webhooks, with guided provider setup and tool approval prompts
...
For a source-of-truth implementation snapshot and the dependency license
inventory, see [`docs/current-implementation-and-components.md`](docs/current-implementation-and-components.md)
and [`docs/OPEN_SOURCE_COMPONENTS.md`](docs/OPEN_SOURCE_COMPONENTS.md).
...
│   │           ├── launch.cpp / launch.h  # Page/process launch lifecycle
│   │           ├── app_registry.*         # Built-in app enable/disable registry
│   │           ├── builtin_app_registry.* # Built-in entry definitions
│   │           ├── desktop_app_loader.*   # Dynamic .desktop entry loading
│   │           ├── app_directory_watcher.* # Dynamic app directory monitoring
│   │           ├── ui_launch_page.*       # Home carousel UI
│   │           ├── launcher_ui_runtime.*  # LVGL runtime/home bootstrap
│   │           ├── animation/             # Animation effects
│   │           ├── model/                 # Testable UI state and policies
│   │           └── page_app/              # Built-in app pages and views
...
- Built-in entries on all builds: Python, Store, CLI/ST terminal, Game, Setting, Math, and LoRa; Linux device builds additionally provide IP Panel, SSH, and Tank Battle
...
- Keyboard-first chat UI for the 320×170 display
- Guided first-run setup for OpenAI, OpenRouter, Anthropic, Ollama, DeepSeek, and custom providers
- ZeroClaw webhook pairing, bearer/secret authentication, and persistent local configuration
- Interactive approval prompts for tool requests returned by the agent
- Runtime FreeType fonts with `ZCLAW_FONT` and `ZCLAW_FALLBACK_FONT` overrides
- Device, Linux SDL2 simulator, Linux cross-compile, and macOS cross-compile configurations
...
For optional local test automation (key injection and framebuffer screenshots), see [APPLaunch Local Automation RPC](docs/automation-rpc.md).
...
The cross-build output is `dist/M5CardputerZero-APPLaunch` and runs on the CardputerZero device. The first cross-build may download the SDK sysroot package into `SDK/github_source/static_lib_v0.0.6`.
...
APPLaunch registers Python, Store, CLI/ST terminal, Game, Setting, Math, and LoRa on all builds. Linux device builds additionally register IP Panel, SSH, and Tank Battle. Python, Store, and Math are command or external-process entries; the others are in-process pages.
